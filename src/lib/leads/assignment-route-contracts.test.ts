...
describe('lead assignment route contract', () => {
  ...
  it('applies both ownership filters', () => {
    expect(route).toMatch(/applyAssigneeFilter\(\s*query,\s*'setter'/);
    expect(route).toMatch(/applyAssigneeFilter\(\s*query,\s*'closer'/);
  });

  it('parses them from the request rather than trusting raw input', () => {
    expect(route).toMatch(/parseAssigneeFilter\(searchParams\.get\('assigned_setter'\)\)/);
    expect(route).toMatch(/parseAssigneeFilter\(searchParams\.get\('assigned_closer'\)\)/);
  });
});

describe('lead assignment page contract', () => {
  it('sends both filters when fetching', () => {
    expect(page).toMatch(/params\.set\('assigned_setter', assignedSetter\)/);
    expect(page).toMatch(/params\.set\('assigned_closer', assignedCloser\)/);
  });

  // A CSV that disagrees with the screen is worse than no export.
  it('carries them into the export', () => {
    const exportBlock = page.slice(page.indexOf('function applyFilterParams'));
    expect(exportBlock.slice(0, 900)).toMatch(/assigned_setter/);
    expect(exportBlock.slice(0, 900)).toMatch(/assigned_closer/);
  });
  ...
  it('refetches when either filter changes', () => {
    expect(page).toMatch(/assignedSetter,\s*assignedCloser\]\)/);
  });
