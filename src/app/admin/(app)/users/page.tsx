  ...
  async function fetchUsers() {
    ...
    try {
      ...
      if (!usersRes.ok || !usersData?.success) {
        throw new Error(usersData?.error || 'Could not load team members');
      }
      setUsers(usersData.users);
    } catch (cause) {
      const message = cause instanceof Error ? cause.message : 'Could not load team members';
      setError(message);
      toast.error(message);
    } finally {
      setLoading(false);
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <DataErrorState compact title="Team members did not load" description={error} onRetry={fetchUsers} />
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
