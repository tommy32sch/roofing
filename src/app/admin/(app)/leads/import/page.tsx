    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {marketsError && (
            <div role="alert" className="rounded-md border border-destructive/25 bg-destructive/5 p-3 text-sm">
              Office data did not load. Try again from the workspace notice before importing.
            </div>
          )}
          ...
          <Button onClick={handleImport} disabled={!file || loading || marketMissing} className="w-full">
            {loading
              ? 'Importing…'
              : marketMissing
                ? marketsError ? 'Office data is unavailable' : 'Choose a market first'
                : file
                  ? `Import ${file.name}`
