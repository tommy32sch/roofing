...
export default function PerformancePage() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      fetch(`/api/admin/performance?${paramsKey}`).then((r) => r.json()),
      fetch('/api/admin/auth/me').then((r) => r.json()),
    ])
      .then(([perf, me]) => {
        if (cancelled) return;
        if (perf.success) setReps(perf.reps);
        if (me.success) {
          setUserRole(me.admin.role);
          setUserId(me.admin.id);
        }
      })
      .catch(() => {})
      .finally(() => {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-center text-xs text-muted-foreground">
        ...
        {userRole !== 'admin' && ' Conversion is based on leads assigned to you.'}
      </p>
    </div>
  );
}
