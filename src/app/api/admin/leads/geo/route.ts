...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      .from('leads')
      .select(
        'id, market_id, first_name, last_name, latitude, longitude, status, priority, estimated_roof_value, address_street, address_city, is_dnc, hail_date, hail_size_inches, last_knock_at, last_disposition, knock_count, do_not_knock, follow_up_date'
      )
